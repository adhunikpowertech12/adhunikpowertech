import WhatIsAirWasher from "./WhatIsAirWasher";


export const metadata = {
  title: "What is an Air Washer ? A Guide to Air Cooling & Purification | Adhunik Powertech",
  description:
    "Learn what an air washer is, how it works, and its benefits. Our guide covers key features, the technology behind industrial air cooling & purification, and ideal applications for industrial and commercial spaces.",

  keywords: [
    
    "what is an air washer",
    "air washer explained",
    "air washing unit",
    "how air washers work",
    "industrial air cooling",
    "air purification system", 
    "air washer benefits",
    "industrial air quality improvement",
    "evaporative cooling technology",
    "humidity control system",
    "air washer for factory",
    "air washer for warehouse",
    "commercial air cooling", 
    "air washer cooling pads",
    "air washer motor",
    "SS 304 air washer",
    "galvanized steel air washer",
    "EC fan technology air washer",
    "air washer vs ducted air cooler", 
    "Adhunik Powertech",
    "air washer manufacturer Delhi NCR",
    "industrial cooling solutions Gurugram",
    "air washing unit supplier Noida",

  ],
  
  openGraph: {
    title: "What is an Air Washer ? A Guide to Air Cooling & Purification | Adhunik Powertech",
    description:
      "Learn what an air washer is, how it works, and its benefits. Our guide covers key features, the technology behind industrial air cooling & purification, and ideal applications for industrial and commercial spaces.",
    url: "https://www.adhunikpowertech.com/what-is-an-air-washer", 
    type: "article",
    images: [
      {
        url: "https://www.adhunikpowertech.com/images/what-is-air-washer-hero.jpg", 
        width: 1200,
        height: 630,
        alt: "A diagram explaining how an air washer unit cools and purifies air using evaporative technology.",
      },
    ],
  },
  alternates: {
    canonical:
      "https://www.adhunikpowertech.com/what-is-an-air-washer", 
  },
  twitter: {
    card: "summary_large_image",
    title: "What is an Air Washer ? A Guide to Air Cooling & Purification | Adhunik Powertech",
    description:
      "Learn what an air washer is, how it works, and its benefits. Our guide covers key features, the technology behind industrial air cooling & purification, and ideal applications for industrial and commercial spaces.",
    image: "https://www.adhunikpowertech.com/blog/1.jpeg",  
  },
};


export default function page() {
    
  return (
    <>
<WhatIsAirWasher/>
    </>
  )
}
