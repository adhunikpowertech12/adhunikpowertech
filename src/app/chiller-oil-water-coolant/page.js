import ChillerOil from "./ChillerOil";



 

export const metadata = {

  title: "Hydraulic Oil Chillers Manufacturer in Delhi NCR",

  description: "Adhunik Powertech: Leading hydraulic oil chiller Manufacturer in Delhi NCR & India. Boost productivity for CNC machines & industrial systems with our high-efficiency solutions.",

  keywords: [
    "Hydraulic Oil Chiller",
    "Oil Chiller Manufacturer",
    "Industrial Oil Chiller",
    "Coolant Chiller",
    "CNC Machine Oil Chiller",
    "Hydraulic Press Chiller",
    "VFD Panel Cooling Oil Chiller",
    "Oil Cooling Plant",
    "Adhunik Powertech Oil Chiller",
    "Energy Efficient Oil Chiller",
    "Low Maintenance Oil Chiller",
    "Durable Oil Chiller",
    "Custom Oil Chiller",
    "Oil Chiller for Machine Tools",
    "Oil Chiller for Hydraulic Power Units",
    "Oil Chiller for Lubrication Systems",
    "Oil Chiller for Process Cooling",
    "Oil Chiller for Injection Molding",
    "Oil Chiller for Die Casting",
    "Oil Chiller for Quenching",
    "Oil Chiller for Power Generation",
    "Delhi NCR Oil Chiller",
    "Gurugram Oil Chiller",
    "Faridabad Oil Chiller",
    "Noida Oil Chiller",
    "Greater Noida Oil Chiller",
    "Ghaziabad Oil Chiller",
    "Sonipat Oil Chiller",
    "Panipat Oil Chiller",
    "Rohtak Oil Chiller",
    "Rewari Oil Chiller",
    "Palwal Oil Chiller",
    "Bhiwadi Oil Chiller",
    "Meerut Oil Chiller",
    "Neemrana Oil Chiller",
    "Tapukara Oil Chiller",
    "Bawal Oil Chiller",
    "Manesar Oil Chiller",
    "Dharuhera Oil Chiller",
    "Muzaffarnagar Oil Chiller",
    "Jaipur Oil Chiller",
    "Chandigarh Oil Chiller",
    "Industrial Cooling India",
    "Best Industrial Oil Chiller",
    "Reliable Oil Cooling Solutions",
    "Oil Chiller Service & Support",
    "Precision Oil Temperature Control",
    "Oil Chiller for Automation",
  ],
  
  robots: "index, follow",

  openGraph: {
    title: "Hydraulic Oil Chiller Manufacturer in Delhi NCR | Adhunik Powertech",
    description: "Adhunik Powertech is a leading hydraulic oil chiller Manufacturer in Delhi NCR & India. Boost productivity for CNC machines & industrial systems with our high-efficiency solutions.",
    url: "https://www.adhunikpowertech.com/chiller-oil-water-coolant", 
    type: "article",
    images: [
      {
        url: "/awi/16.jpeg", 
        width: 800,
        height: 600,
        alt: "Adhunik Powertech Hydraulic Oil Chiller Units",
      },
    ],
  },
  alternates: {
    canonical: "https://www.adhunikpowertech.com/chiller-oil-water-coolant", 
  },
  twitter: {
    card: "summary_large_image",
    title: "Hydraulic Oil Chiller Manufacturer in Delhi NCR | Adhunik Powertech",
    description: "Need a reliable hydraulic oil chiller? Adhunik Powertech is a leading Manufacturer in Delhi NCR & India. Explore our high-efficiency industrial oil chiller solutions.",
    image: "/awi/16.jpeg",
  },
};


export default function DuctedAirCooler() {

  return (
    <>

      <ChillerOil />

    </>

  )
}
