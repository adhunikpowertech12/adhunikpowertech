import HvacProduct from "./HvacProduct";




export const metadata = {

  title: "HVAC Contractor Gurugram | Turnkey HVAC Solutions - Adhunik Powertech",

  description: "Adhunik Powertech: Your trusted HVAC contractor in Gurugram & Delhi NCR. We deliver turnkey HVAC solutions for commercial & industrial needs. 19+ years experience, award-winning service.",



  keywords: [

    "HVAC Contractor",
    "HVAC Company",
    "HVAC Services",
    "Heating and Cooling Contractor",
    "Turnkey HVAC Solutions",
    "HVAC Installation Contractor",
    "HVAC System Design",
    "Commercial HVAC Contractor",
    "Industrial HVAC Contractor",
    "HVAC Contractor Gurugram",
    "HVAC Company Gurugram",
    "HVAC Services Gurugram",
    "HVAC Contractor Delhi NCR",
    "HVAC Company Delhi NCR",
    "HVAC Services Delhi NCR",
    "HVAC Contractor Haryana",
    "HVAC Installation Gurugram",
    "Commercial HVAC Gurugram",
    "Industrial HVAC Delhi",
    "Chiller Plant Installation",
    "Air Washer Installation",
    "Panel AC Installation",
    "Ducted Air Cooler Installation",
    "Clean Room HVAC Contractor",
    "Basement Ventilation System",
    "Lift Pressurization System",
    "HVAC Maintenance Services",
    "HVAC Annual Maintenance Contract",
    "Adhunik Powertech HVAC",
    "Adhunik Powertech Services",
    "ISO Certified HVAC",
    "Energy Efficient HVAC",
    "Reliable HVAC Contractor",
    "Award Winning HVAC",
  ],
  robots: "index, follow",

  openGraph: {
    title: "HVAC Contractor Gurugram | Turnkey HVAC Solutions - Adhunik Powertech",
    description: "Adhunik Powertech: Your trusted HVAC contractor in Gurugram & Delhi NCR. We deliver turnkey HVAC solutions for commercial & industrial needs. 19+ years experience, award-winning service.",
    url: "https://www.adhunikpowertech.com/hvac",
    type: "website",
    images: [
      {
        url: " https://res.cloudinary.com/ddkyx2jhh/image/upload/v1748256475/hvac2_v4rz2t.webp",
        width: 1200,
        height: 630,
        alt: "Adhunik Powertech HVAC Contractor Team working on a Turnkey Solution",
      },
    ],
  },
  alternates: {
    canonical: "https://www.adhunikpowertech.com/hvac",
  },

  twitter: {
    card: "summary_large_image",
    title: "HVAC Contractor Gurugram | Turnkey HVAC Solutions - Adhunik Powertech",
    description: "Need a reliable HVAC contractor in Gurugram/Delhi NCR? Adhunik Powertech offers 19+ years experience in turnkey HVAC solutions for commercial & industrial sectors.",
    image: " https://res.cloudinary.com/ddkyx2jhh/image/upload/v1748256475/hvac2_v4rz2t.webp",
  },
};


export default function Page() {

  return (
    <>
      <HvacProduct />

    </>
  );
}
